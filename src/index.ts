...
app.get(`/`, async (request, response) => {
	...
	if (request.query.status) {
		...
		if (status === `0`) {
			...
			// User has already claimed Bunkercoins
			...
			const dateToClaim_address = db
				.prepare(`SELECT expires FROM limitByAddress WHERE address=?`)
				.get(request.query.address) as { expires: number };
	...
	if (dateToClaim_ip) {
		if (dateToClaim_ip.expires < Date.now()) {
			...
			response.status(200).render(`error.ejs`, {
				error: `Hi! It looks like you've already claimed your Bunkercoins for today! Please come back at ${new Date(
					dateToClaim_ip.expires,
				).toUTCString()}!`,
			});
			return;
		}
...
app.post(`/receive`, async (request, response) => {
	...
	if (
		...
		!(await helper.verifyHcaptcha(request.body[`h-captcha-response`]))
	) {
	...
	if (/^[B][a-zA-Z0-9]{33}$/.test(address)) {
		...
		const byAddress = db.prepare(`SELECT expires FROM limitByAddress WHERE address=?`).all(address) as {
			expires: number;
		}[];
		...
		db.prepare(`INSERT INTO limitByAddress (address, expires) VALUES (?, ?)`).run(
			address,
